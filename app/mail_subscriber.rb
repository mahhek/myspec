class MailSubscriber
  # This script deletes all posts that are over 5 minutes old
    ...
    subscriptions =  Subscription.all(:conditions => "CONVERT(payment_on, date) = '#{Time.now.to_date}'" )
        ...
        if user.active && !(user.is_suspended) && !(user.cancel_subscription)
          ...
          if response.success?
            ...
            if response.success?
              ...
              #next_payment_on = payment_on.to_time.advance(:months => 1).to_date
              ...
              subretry.retry_count = 1
              subretry.retry_on = Time.now.to_time.advance(:days => 1).to_date
              subretry.save
              Notifier.deliver_initial_try_failed(user)
            end
          end
        end
      end
    end
    puts "send cron email"
end